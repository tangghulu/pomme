    ...
    >
      <div className="max-w-md mx-auto w-full flex flex-col flex-1 px-5 pt-5 pb-8">
        <div className="flex items-center justify-between mb-2">
          <button onClick={back} className="w-10 h-10 rounded-2xl bg-muted flex items-center justify-center hover:bg-accent transition-colors">
        ...
        <div className="flex-1">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              transition={{ duration: 0.25, ease: [0.25, 0.1, 0.25, 1] }}
            >
              {step === 0 && <StepNameAndDays choreName={choreName} setChoreName={setChoreName} selectedDays={selectedDays} toggleDay={toggleDay} />}
              ...
              {step === 3 && <StepReview choreName={choreName} selectedDays={selectedDays} frequency={frequency} reminderTime={reminderTime} peopleNeeded={peopleNeeded} autoRotate={autoRotate} />}
            </motion.div>
          </AnimatePresence>
        </div>
        ...
        <div className="mt-6">
          {step < TOTAL_STEPS - 1 ? (
            ...
            <button onClick={handleCreate} className="w-full py-4 rounded-2xl font-bold text-base bg-primary text-primary-foreground shadow-md hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 flex items-center justify-center gap-2">
...
function StepNameAndDays({ choreName, setChoreName, selectedDays, toggleDay }: { choreName: string; setChoreName: (v: string) => void; selectedDays: string[]; toggleDay: (d: string) => void }) {
  return (
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">Automatically rotate this chore among roommates each cycle so no one gets stuck.</p>
            </div>
          </div>
...
function StepReview({ choreName, selectedDays, frequency, reminderTime, peopleNeeded, autoRotate }: { choreName: string; selectedDays: string[]; frequency: string; reminderTime: string; peopleNeeded: number; autoRotate: boolean }) {
